...
<head>
    ...
    <title>Lead Assignment | CRM Admin</title>
    ...
    <style>
        :root { --primary: #2563eb; --border: #e5e7eb; --text-dark: #111827; --text-muted: #6b7280; --surface: #f8fafc; }
        ...
        .la-tab-btn.active { background: #eff6ff; color: #1d4ed8; }
        ...
        [data-theme="dark"] .la-tab-btn.active { background: rgba(147,164,253,.16); color: #93a4fd; }
...
<body>
            ...
            <div class="content-wrapper">
                ...
                <div class="crm-page-header">
                    <div class="crm-header-left">
                        ...
                        <div>
                            <h4>Lead Assignment Rules</h4>
                            <small class="crm-subtitle">Configure automatic lead assignment rules. Priority: Product &rarr; State &rarr; Source &rarr; Round Robin.</small>
                        </div>
                    </div>
                </div>

                <div class="la-tabs" id="laTabs">
                    <button class="la-tab-btn active" data-tab="product">Product Rules</button>
                    <button class="la-tab-btn" data-tab="state">State Rules</button>
                ...
                <div class="la-panel active" id="panel-product">
                    <div class="la-panel-head">
                        <p>Assign leads to agents based on product matching. Multiple agents use round-robin.</p>
                        @can('leads.manage-settings')
                        <button class="btn btn-primary btn-sm add-rule-btn" data-type="product"><i class="fa fa-plus"></i> Add Rule</button>
                        @endcan
                    </div>
                    <div class="la-card" id="list-product"></div>
                </div>

                <div class="la-panel" id="panel-state">
                    <div class="la-panel-head">
                        <p>Assign leads to agents based on the lead's state. Multiple agents use round-robin.</p>
                        @can('leads.manage-settings')
                        <button class="btn btn-primary btn-sm add-rule-btn" data-type="state"><i class="fa fa-plus"></i> Add Rule</button>
                        @endcan
                    </div>
                    <div class="la-card" id="list-state"></div>
                </div>

                <div class="la-panel" id="panel-source">
                    <div class="la-panel-head">
                        <p>Assign leads to agents based on where the lead came from. Multiple agents use round-robin.</p>
                        @can('leads.manage-settings')
                        <button class="btn btn-primary btn-sm add-rule-btn" data-type="source"><i class="fa fa-plus"></i> Add Rule</button>
                        @endcan
                    </div>
                ...
                <div class="la-panel" id="panel-round_robin">
                    <div class="la-panel-head">
                        <p>The final fallback — any lead that doesn't match a Product, State, or Source rule goes to this pool, round-robin.</p>
                        @can('leads.manage-settings')
                        <button class="btn btn-primary btn-sm add-rule-btn" data-type="round_robin"><i class="fa fa-plus"></i> Set Pool</button>
                        @endcan
                    </div>
                ...
                <form id="ruleForm">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group" id="matchValueGroup">
                            <label id="matchValueLabel">Product</label>
                            <input type="text" name="match_value" id="matchValue" class="form-control" placeholder="e.g. CNC Machine">
                            <select id="matchValueSelect" class="form-control" style="display:none;"></select>
                        </div>
    ...
    <script>
        ...
        const TAB_LABELS = { product: 'Product', state: 'State', source: 'Source', round_robin: 'Round Robin' };
        ...
        function renderRuleRow(rule, type) {
            ...
            const valueLabel = type === 'round_robin' ? 'Everyone else' : esc(rule.match_value);
            return `
        ...
        function emptyState(type) {
            const copy = {
                product: ['No Product rules', 'Create a rule to auto-assign leads based on product.'],
                state: ['No State rules', 'Create a rule to auto-assign leads based on state.'],
                source: ['No Source rules', 'Create a rule to auto-assign leads based on source.'],
                round_robin: ['No fallback pool set', 'Set a pool so leads matching nothing else still get assigned automatically.'],
            }[type];
        ...
        function load() {
            $.get("{{ route('lead_assignment.data') }}", function (res) {
        ...
        function openModal(type, rule) {
            ...
            if (type === 'source') {
                $('#matchValue').hide(); $('#matchValueSelect').show();
                let opts = '<option value="">Select a source…</option>';
                LEAD_SOURCES.forEach(s => { opts += `<option value="${esc(s.code)}">${esc(s.label)}</option>`; });
                $('#matchValueSelect').html(opts);
                if (rule) $('#matchValueSelect').val(rule.match_value);
            } else {
                $('#matchValueSelect').hide(); $('#matchValue').show();
                if (rule) $('#matchValue').val(rule.match_value);
            }
        ...
        $(document).on('click', '.delete-rule-btn', function () {
            const id = $(this).data('id');
            if (!confirm('Remove this rule? Leads will fall through to the next priority level instead.')) return;
            $.ajax({ url: `{{ url('settings/lead-assignment') }}/${id}`, method: 'DELETE' })
        ...
        $(document).on('click', '.toggle-rule-btn', function () {
            ...
            $.ajax({ url: `{{ url('settings/lead-assignment') }}/${id}`, method: 'PUT', data: { is_active: nowActive ? 1 : 0 } })
                .done(() => load())
        ...
        $('#ruleForm').on('submit', function (e) {
            ...
            const id = $('#ruleId').val();
            const matchValue = type === 'source' ? $('#matchValueSelect').val() : $('#matchValue').val();
            const agentIds = $('#agentPicker input[type="checkbox"]:checked').map(function () { return this.value; }).get();

            if (type !== 'round_robin' && !matchValue) { toastr.error('Enter a value to match on.'); return; }
            ...
            const url = id ? `{{ url('settings/lead-assignment') }}/${id}` : "{{ route('lead_assignment.store') }}";
