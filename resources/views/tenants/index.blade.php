...
<div class="collapse" id="newCompany"><form method="post" action="{{ route('superadmin.tenants.store') }}">@csrf<div class="form-row">
@foreach([['name','Company name','text'],['contact_email','Contact email','email'],['admin_name','Admin name','text'],['admin_email','Admin email','email'],['admin_phone','Admin phone','text'],['admin_password','Admin password','password']] as $field)<div class="col-md-4 mb-2"><input class="form-control" type="{{ $field[2] }}" name="{{ $field[0] }}" placeholder="{{ $field[1] }}" {{ in_array($field[0],['name','admin_name','admin_email','admin_password'])?'required':'' }}></div>@endforeach
<div class="col-md-3"><select class="form-control" name="plan"><option value="trial">Trial</option><option value="standard">Standard</option><option value="professional">Professional</option><option value="enterprise">Enterprise</option></select></div><div class="col-md-3"><input class="form-control" name="timezone" value="Asia/Kolkata" required></div><div class="col-md-2"><input class="form-control" name="locale" value="en" required></div><div class="col-md-2"><input class="form-control" type="number" min="1" name="max_users" placeholder="User limit"></div><div class="col-md-2"><input class="form-control" type="date" name="trial_ends_at"></div><div class="col-md-2"><button class="btn btn-primary">Create & provision</button></div></div></form></div>
</div></div>
