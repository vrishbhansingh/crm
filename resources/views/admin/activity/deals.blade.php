...
@section('content')
    ...
    <button type="button" class="btn btn-warning" style="padding: 9px 17px; border-radius: 2px;background-color: revert;" data-toggle="modal" data-target="#exampleModal-4" data-whatever="@mdo" title="Add User">
        Add Deals&nbsp;<i class="fa fa-cube"></i>
    </button>
</div>
  ...
  <div class="modal-dialog" role="document"> <!-- Default medium size -->
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="ModalLabel" style="font-size: 19px;">Add Deals</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      
      <form class="form-sample add_deal" id="add_deal" method="post">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="mb-3">
                ...
                <input type="date" name="expact_close_date" 
                       class="form-control" style="border-radius: 5px;">
              </div>
              
              <div class="mb-3">
            <label class="form-label">Stage</label>
            <select name="stage" class="form-control" style="border-radius: 5px; height: 50px; font-size: 16px;">
            ...
            <div class="col-md-6">
              <div class="mb-3">
                ...
                <input type="text" name="deal_name" placeholder="Deal Name"
                       class="form-control" style="border-radius: 5px;">
              </div>
    ...
    $('#order_list').dataTable(
        {
            ...
            "columns": [
               {"targets": 0, "name": "id", 'searchable': false,  'orderable': true},
                {"targets": 1, "name": "lead_id", 'searchable': false,  'orderable': false},
                {"targets": 2, "name": "deal_name", 'searchable': false, 'orderable': false},
                {"targets": 3, "name": "amount", 'searchable': false, 'orderable': true},
                {"targets": 4, "name": "close_date", 'searchable': false, 'orderable': true},
                {"targets": 5, "name": "stage", 'searchable': false, 'orderable': true},
...
$('#add_deal').on('submit', function(e){
            e.preventDefault();
      
            var fd = new FormData(this);
                fd.append('_token','{{csrf_token()}}');
                $.ajax({
                    url: "{{ route('admin.save_deal_enquiry') }}",
                    type: "POST",
                    data: fd,
                    dataType: 'json',
                    processData: false,
...
$('body').on("click", ".del-deal", function (e) {
					...
					$.confirm({
						title: 'Confirm!',
						content: 'Sure you want to delete this Deal?',
						buttons: {
							yes: function () {
								$.ajax({
									url: "{{ route('admin.del_deal') }}/",
									type: 'POST',
									data: fd,
									dataType: "JSON",
									contentType: false,
