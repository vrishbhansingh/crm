...
class LeadIntegrationController extends Controller
{
    ...
    public function index()
    {
        ...
        $integrations = LeadIntegration::where('tenant_id', $tenantId)->latest()->get()->keyBy('platform');
        $platforms = self::PLATFORM_META;

        return view('integrations.index', compact('integrations', 'platforms'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->validate([
            'platform' => ['required', Rule::in(self::PLATFORMS), Rule::unique('lead_integrations', 'platform')->where('tenant_id', $tenantId)],
        ...
        $integration = LeadIntegration::create([
            ...
            'verify_token' => in_array($data['platform'], ['facebook', 'whatsapp'], true) ? \Illuminate\Support\Str::random(32) : null,
            'created_by' => Auth::guard('web')->id(),
        ]);

        return response()->json(['status' => true, 'message' => 'Integration connected.', 'data' => ['id' => $integration->id]]);
    }
    ...
    public function update(Request $request, LeadIntegration $integration)
    {
        ...
        return response()->json(['status' => true, 'message' => 'Integration updated.']);
    }
    ...
    public function regenerate(LeadIntegration $integration)
    {
        ...
        $integration->update([
            'token' => LeadIntegration::generateToken(),
            'verify_token' => in_array($integration->platform, ['facebook', 'whatsapp'], true) ? \Illuminate\Support\Str::random(32) : $integration->verify_token,
        ]);
